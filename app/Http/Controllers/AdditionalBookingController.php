...
class AdditionalBookingController extends Controller
{
    ...
    public function index()
    {
        ...
        $orders = DB::table('orders')->where('phone', auth()->guard('web')->user()->phone)->where('payment_status', 'paid')->get();
        
        $total_order = 0;
        foreach ($orders as $order) {
            $order_items = DB::table('order_items')->where('order_id', $order->id)->get();
            $total_order+= count($order_items);
        }
        ...
        return view('web.additional_bookingform')->with(compact('meta','slots','classes','dates','price','plan', 'states', 'cities', 'orders','total_order'));        
    }
    ...
    function slot_default_price()
    {
        ...
        $price = 0; //for additional slot booking 
        return $price;
    }
